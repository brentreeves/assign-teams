...
class Buckets:
    ...
    def setupFolks(self):
        ...
        self.log(2, "setupFolks has %d" % len(self.folks))
    ...
    def swap (self, i, j):
        self.log(3, "swap %d %d folks: %d" % (i, j, len(self.folks)))
        x = self.folks[i]
    ...
    def score_pref (self, spot, spot2, aList):
        ...
        for n in range(0, len(myprefs)-1):
            ...
            if self.prefs[ person1-1 ][n+1] == person2:
                ...
                return self.rank[n]
        self.log(3, "  score_pref return %d-%d absent  %d-%d RATES 0" % (spot, spot2, person1, person2))
        return 0
    ...
    # return the number of students in this team who have zero happinesses
    def score_team_zeros(self, team, aList):
        ...
        for i in range(0, self.teamsize):
            ...
            self.log(4,"score_team+zeros calling score_one: %d  with: %s" % (n, str(aList)))
    ...
    def pprint(self, aList):
        for i in range(0, len(self.folks)):
            if i % self.teamsize == 0:
                ...
                print("Team %d (%s) Score: %2d %d zeros " % (team, str(teamNumbers), self.score_team( team, aList), zeros), end='')
    ...
    def goodTrade(self, zeros_before1, zeros_before2, zeros_after1, zeros_after2):
        ...
        # print("goodTrade %d v %d | %d v %d and %d v %d" % ((zeros_before1 + zeros_before2), (zeros_after1 + zeros_after2), zeros_before1, zeros_after1, zeros_before2, zeros_after2))
        return False
    ...
    def run(self):
        ...
        print("Best %d: %s overrides: %d  values: %s" % ( self.super_score, str(self.super_folks), self.countOverides, str(self.rank)))
    ...
    def scorenow(self, title, alist, swaps, nixed):
        ...
        print("%s %d (%d zeros): %s swaps: %d (%d)" % (title, t, z, str (alist), swaps, nixed))
