# File: graph-weight.py  Mr Blee's prefs grapher
# Author: Brent Reeves
# Copyright: (c) 2018 Brent Reeves
# Date: Fall 2018
# Input: team member preference list:  Name,int,int,int,int...  1-based preferences in order
# Output: dot graph
# How: loop through each prefs list, generating arcs
# Run: python graph-weight-cols.py -v 0 < prefs.txt
#
import sys
import random
import argparse
...
class Arcs:
    ...
    def printArcsWeighted(self):
        self.log(2,"printArcsWeighted()")
        self.arcs = []
        for s in self.prefs:
            self.log(2,"prefs s: %s" % s)
            weight = 6
            for p in s[1:]:
                if (p > 0):
                    print( "%s,%s,%d" % (s[0], self.prefs[p-1][0], weight))
                    weight -= 1
    ...
    def printGraph(self):
        self.log(2,"printGraph()")
        self.printHeader()
        self.printArcsWeighted()
        self.printEnd()
